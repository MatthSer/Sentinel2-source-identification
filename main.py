...
def main(reference, test, patch_size, filter, crop=False, crop_size=None):

    # Compute profiles
    s1, _, _ = get_profil(reference, filter)
    s2, x_start, y_start = get_profil(test, filter, crop, crop_size)

    # Compare signals
    if len(s1) == len(s2):
        p_value_gauss, p_value_rank, list_corr, list_corr_ij = prnu_similarity_profil(s1, s2, patch_size)
        ...
        idx2 = results['idx2']
        offset = results['lag']
        p_value_gauss, p_value_rank, list_corr, list_corr_ij = prnu_similarity_profil(s1_crop, s2_crop, patch_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--patch_size", type=int, default=20, required=False)
    ...
    main(args.reference, args.test, args.patch_size, args.filter, args.crop, args.crop_size)
